...
where $F_t$ is effectively an exploitation rate. 

### Continuous surplus production

Smaller time steps are used in the model to approximate continuous production and fishing. With the biomass at the start of the year, the model iteratively solves for the year-specific fishing mortality rate $F_t$ that produces the observed annual catch over sub-annual time steps.

By default, four sub-annual time steps are used with duration $\Delta = 0.25$ and $t'=1,2,3,4$.

For each seasonal time step $t'$ within year $t$:
...
$$L^r = - 0.5\left(\dfrac{\log(\hat{r}) - \log(\mu_r)}{\sigma_r}\right)^2,$$
Users can explicitly specify $\log(\mu_r)$ and $\sigma_r$. 
