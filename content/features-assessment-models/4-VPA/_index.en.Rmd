...
Fishery selectivity $v_{a,t}$ can derived from $F_{a,t}$:
$$v_{a,t} = \dfrac{F_{a,t}}{\max \limits_a F_{a,t}}$$
...
$$ \begin{align}
\textrm{pen}_N &= \sum_{i=T-\tilde{t}}^T \left(-\log(\sigma_{\tilde{a}}) - 0.5 \left[\dfrac{\log(N_{\tilde{a},i}) - \log(N_{\tilde{a},i-1})}{\sigma_{\tilde{a}}}\right]^2\right) \\
\textrm{pen}_v &= \sum_{i=T-\tilde{t}}^T \left(-\log(\sigma_v) - 0.5 \left[\dfrac{\log(v_{a,i}) - \log(v_{v,i-1})}{\sigma_v}\right]^2\right) 
\end{align},$$
where $\tilde{a}$ is the smallest age in the VPA.
