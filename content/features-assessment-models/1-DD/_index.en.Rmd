...
$$ s_t = (1 - u_t)\exp(-M), $$
where $u_t$ is the annual exploitation rate and $M$ is the instantaneous natural mortality rate. 

#### Conditioning on catch

If the model is conditioned on catch ($C_t$), the exploitation rate $u_t$ is
$$ u_t = C_t/B_t. $$
...
Otherwise, if the model is conditioned on effort (as the ratio of the catch and index), the exploitation rate is
$$ u_t = 1 - \exp(-q^f f_t), $$
