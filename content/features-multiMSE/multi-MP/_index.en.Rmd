...
Now we need to code an `MMP.` The vanilla class of method [`MP`](/features-management-procedures/), takes in data in an object of class [`Data`](/object-data/) and provides management recommendations in an object of class [`Rec`](/object-rec/). The `MMP` works exactly the same way but it accepts `Data` objects in a hierarchical list (Fleets nested in Stocks) and provides MP recommendations in a hierarchical list of the same dimensions (Fleets nested in Stocks).
