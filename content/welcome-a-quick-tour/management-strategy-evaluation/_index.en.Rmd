...
For example, the slot `SB_SBMSY` is a 3-dimensional array with the spawning biomass ($\text{SB}$) relative to the spawning biomass corresponding to maximum sustainable yield ($\text{SB}_{\text{MSY}}$; see [Reference Points](/tutorial-reference-points/) for more details) for each simulation, MP, and projection year:
